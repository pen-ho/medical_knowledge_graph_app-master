...
def train(args, train_dataset, model, tokenizer):
    ...
    bert_param_optimizer = list(model.bert.named_parameters())  # 把模型涉及的学习参数都拿到 hzp
    crf_param_optimizer = list(model.crf.named_parameters())  # 把模型涉及的学习参数都拿到
    linear_param_optimizer = list(model.classifier.named_parameters())  # 把模型涉及的学习参数都拿到
    # 对于偏置和归一化层的权重 不设置权重衰减，其余的参数设置指定的权重衰减
    ...
    # 构建优化器[类型：torch.optim.optimizer]，传入前面设置好的不同层的各组优化器参数组{params: weight_decay: lr:}
    # eps 在分母上加一项增加数值稳定性
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    # 对学习率设置一个自动调节器scheduler：lr先线性上升后线性下降 [类型：torch.optim.lr_scheduler]
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps,
                                                num_training_steps=t_total)
    # Check if saved optimizer or scheduler states exist
    if os.path.isfile(os.path.join(args.model_name_or_path, "optimizer.pt")) and os.path.isfile(
            os.path.join(args.model_name_or_path, "scheduler.pt")):
        # Load in optimizer and scheduler states [优化器和调节器调用state_dict()的输出保存到.pt文件]
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            tags = model.crf.decode(logits, inputs['attention_mask'])  # 通过crf层
        if args.n_gpu > 1:
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_crf-{}_{}_{}_{}'.format(
        ...
        str(task)))
    if os.path.exists(cached_features_file) and not args.overwrite_cache and 1 == 0:  # 不检查cache的数据，每次从数据集文件读出
        ...
        # examples = examples[:200] # 调试
        # print("调试"+str(examples))
