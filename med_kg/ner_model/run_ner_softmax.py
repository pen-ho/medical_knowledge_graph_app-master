...
from callback.progressbar import ProgressBar  # 自定义的进度条
...
def evaluate(args, model, tokenizer, prefix=""):
        ...
        for i, label in enumerate(out_label_ids):
            ...
            for j, m in enumerate(label):
                if j == 0:
                    ...
                    metric.update(pred_paths=[temp_2], label_paths=[temp_1])  # 调用类的方法写入预测序列和标签序列
                    break
                else:
    ...
    eval_info, entity_info = metric.result()  # 调用函数对对象的属性进行性能评估
